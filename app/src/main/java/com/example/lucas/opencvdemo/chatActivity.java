...
public class chatActivity extends Activity implements OnItemClickListener ,OnClickListener,CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        synchronized (inputFrame) {
           ...
           // Imgproc.cvtColor(_rgbaImage,_hsvMat, Imgproc.COLOR_BGR2BGRA);
          //  Imgproc.cvtColor(_rgbaImage, _hsvMat, Imgproc.COLOR_BGR2BGRA);
            Imgproc.GaussianBlur(_rgbaImage,_hsvMat,new Size(7,7), 2, 2);
            Imgproc.HoughCircles(_hsvMat, circles, Imgproc.CV_HOUGH_GRADIENT, 1, 60, 200, 20, 30, 0 );
            ...
            if (circles.cols() > 0)
                for (int x = 0; x < circles.cols(); x++)
                {
                    ...
                    Core.circle(_rgbaImage, pt, radius, new Scalar(0,255,0),3, 8, 0);
                 //   Core.circle(destination, pt, 3, new Scalar(0,0,255), 3, 8, 0);
                }
